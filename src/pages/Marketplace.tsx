...
const FilterDropdown: React.FC<FilterDropdownProps> = ({ options, value, onChange, placeholder }) => {
  return (
    <select
      ...
      className="border border-[#a8d5b2] text-gray-700 rounded-xl px-3 py-1.5 shadow-sm bg-white/70 backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-[#43a047] transition-all duration-300 hover:border-[#43a047]"
  ...
  useEffect(() => {
    const mockProducts: Product[] = [
      ...
      },
      {
        ...
        description:
          'A beautifully handwoven basket crafted by skilled local artisans using natural, sustainable fibers.',
        price: 29.99,
        stock: 15,
        cooperative: 'Artisan Crafts Coalition',
        image:
          'https://www.azizilife.com/wp-content/uploads/2020/02/20191230-Traditional-Grass-Peace-Basket-White-and-Black-450x450.jpg'
      },
      {
        id: '3',
        title: 'Organic Honey',
        description: 'Raw, unprocessed honey from sustainable beekeepers.',
        price: 9.99,
        stock: 30,
      ...
      },
      {
        ...
        description: 'Creamy, rich, and full of flavor. Perfect for cheese boards or gifting.',
        price: 19.99,
        stock: 25,
  ...
  return (
    <>
      ...
      <div
        style={{
          backgroundImage: `
            radial-gradient(circle at 10% 20%, rgba(173, 232, 190, 0.3), transparent 40%),
            radial-gradient(circle at 90% 80%, rgba(173, 216, 230, 0.3), transparent 40%),
            linear-gradient(to bottom right, #f8fbf9, #eafaf1, #d7f3e3)
          `
        }}
        className="min-h-screen py-8 px-4 bg-fixed bg-cover bg-no-repeat transition-all duration-500"
      >
        <div className="max-w-7xl mx-auto">
          {/* Title Section */}
          <div className="text-center mb-4 mt-16">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-[#2e7d32] to-[#43a047] bg-clip-text text-transparent tracking-tight mb-1">
              Marketplace
            </h1>
            <p className="text-gray-700 font-medium text-base">
              Discover unique products from local cooperatives
            </p>
          </div>
          ...
          {/* Search and Filter */}
          <div className="flex flex-col sm:flex-row gap-3 mb-6 bg-white/50 p-3 rounded-xl backdrop-blur-md border border-[#bde5b8] shadow-sm">
            <SearchBar
              value={search}
              onChange={setSearch}
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="border border-[#a8d5b2] rounded-xl p-5 animate-pulse bg-white/70 shadow-sm"
                />
              ))}
            </div>
          ) : error ? (
            ...
            <div className="text-center py-12 bg-white/60 rounded-xl shadow-sm">
